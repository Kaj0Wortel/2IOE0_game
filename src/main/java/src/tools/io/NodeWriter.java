...
/**
 * Writes {@link Node}s to a text-file.
 */
public class NodeWriter implements Closeable {
     ...
     */
    public NodeWriter(String fileName) throws IOException {
     ...
     * @throws IOException if the file could not be accessed.
     */
    public void writeNodeChain(Node node) throws IOException {
        while (node != null) {
            writeNode(node);
    ...
    public static void main(String[] args) {
        ...
        for (double i = 0; i < 10; i++) {
            nodes.add(
                    new Node(new Point2D.Double(i, i),
                            i,
                            i,
                            i,
                            i,
                            i,
                            i,
                            (int) i, (nodes.isEmpty() ? null : nodes.get(nodes.size() - 1)))
            );
        }
