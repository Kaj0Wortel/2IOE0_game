...
public class ShadowBoxTest {
	 ...
	 *            - the in-game camera.
	 */
	protected ShadowBoxTest(Matrix4f lightViewMatrix, Camera camera) {
	...
	protected void update() {
		...
		Vector4f forward4 = new Vector4f(FORWARD).mul(rotation);
		...
		Vector3f toNear = new Vector3f(forwardVector).mul(Renderer.NEAR);
	...
	private Vector4f[] calculateFrustumVertices(Matrix4f rotation, Vector3f forwardVector,
		...
		Vector4f upVector4f = new Vector4f(UP).mul(rotation);
		Vector3f upVector = new Vector3f(upVector4f.x, upVector4f.y, upVector4f.z);
		Vector3f rightVector = new Vector3f(upVector).cross(forwardVector);
	...
	private void calculateWidthsAndHeights() {
		...
		nearWidth = (float) (Renderer.NEAR
				* Math.tan(Math.toRadians(Renderer.FOV)));
	...
	private float getAspectRatio() {
		return (float) Renderer.width / (float) Renderer.height;
	}
